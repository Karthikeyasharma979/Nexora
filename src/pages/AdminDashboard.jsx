...
const AdminDashboard = () => {
  ...
  const [newTestDuration, setNewTestDuration] = useState(900);
  ...
  const handleEditTest = (test) => {
    ...
    setNewTestRequireSEB(test.requireSEB !== false);
                ...
                <tbody>
                  {tests.map(test => (
                    <tr key={test.id}>
                      ...
                      <td><span className="duration-pill">{test.duration / 60} mins</span></td>
                      <td>{test.questions?.length || 0} Qs</td>
                      <td>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          <button className="btn-action email-btn" onClick={() => openEmailModal(test)} title="Email Secure Link">
                            <Mail size={16} /> Email
                          </button>
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          <button className="icon-btn-action" style={{ color: '#3b82f6' }} onClick={() => handleEditTest(test)} title="Edit Test"><Edit2 size={16} /></button>
                          <button className="icon-btn-action" style={{ color: '#ef4444' }} onClick={() => handleDeleteTest(test.id)} title="Delete Test"><Trash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
                  {tests.length === 0 && <tr><td colSpan="5" className="empty-state">No tests created yet.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
            ...
            <div className="builder-layout">
              <div className="builder-form-section">
                ...
                <div className="form-row mb-4">
                  <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', gap: '8px', fontSize: '14px', fontWeight: '500', marginRight: '20px' }}>
                    <input type="checkbox" checked={newTestRequireCamera} onChange={e => setNewTestRequireCamera(e.target.checked)} style={{ width: '16px', height: '16px' }}/>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', gap: '8px', fontSize: '14px', fontWeight: '500' }}>
                    <input type="checkbox" checked={newTestRequireSEB} onChange={e => setNewTestRequireSEB(e.target.checked)} style={{ width: '16px', height: '16px' }}/>
                    Require Secure Browser (SEB)
                  </label>
                </div>
                ...
                <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', borderBottom: '1px solid #e2e8f0', paddingBottom: '12px' }}>
                  <button type="button" onClick={() => setBuilderMode('manual')} style={{ padding: '8px 16px', fontSize: '13px', fontWeight: '600', borderRadius: '6px', cursor: 'pointer', border: 'none', backgroundColor: builderMode === 'manual' ? '#1e56a0' : 'transparent', color: builderMode === 'manual' ? 'white' : '#64748b', transition: 'all 0.2s' }}>Manual Form</button>
                  <button type="button" onClick={() => setBuilderMode('bulk')} style={{ padding: '8px 16px', fontSize: '13px', fontWeight: '600', borderRadius: '6px', cursor: 'pointer', border: 'none', backgroundColor: builderMode === 'bulk' ? '#1e56a0' : 'transparent', color: builderMode === 'bulk' ? 'white' : '#64748b', transition: 'all 0.2s' }}>✨ Smart Bulk Paste</button>
                </div>
                ...
                {builderMode === 'manual' ? (
                  <form onSubmit={handleAddQuestion} className="question-form">
                    <div className="form-group">
                      ...
                      <textarea className="modern-input" rows="2" value={currentQuestion.text} onChange={(e) => setCurrentQuestion({...currentQuestion, text: e.target.value})} placeholder="Type your question here..." required />
                    </div>
                      ...
                      {currentQuestion.options.map((opt, idx) => (
                        <div key={idx} className={`option-card ${currentQuestion.correctOption === idx ? 'selected' : ''}`}>
                          <div className="radio-wrapper">
                            <input type="radio" name="correctOption" checked={currentQuestion.correctOption === idx} onChange={() => setCurrentQuestion({...currentQuestion, correctOption: idx})} />
                          </div>
                          <input type="text" className="option-input" value={opt} onChange={(e) => handleOptionChange(idx, e.target.value)} placeholder={`Option ${idx + 1}`} required />
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="form-group mt-4" style={{ marginTop: '15px' }}>
                      ...
                      <select className="modern-input" value={currentQuestion.correctOption} onChange={(e) => setCurrentQuestion({...currentQuestion, correctOption: parseInt(e.target.value, 10)})}>
                    ...
                    <div className="form-actions mt-4" style={{ display: 'flex', gap: '10px' }}>
                      <button type="submit" className="btn-secondary"><Plus size={16} /> {editingQuestionIndex !== null ? 'Update Question' : 'Add Question'}</button>
                      {editingQuestionIndex !== null && (
                        <button type="button" className="btn-secondary" onClick={handleCancelEditQuestion} style={{ backgroundColor: '#f1f5f9', color: '#475569', borderColor: '#cbd5e1' }}>Cancel Edit</button>
                      )}
                    </div>
                  </form>
                ) : (
                  <div className="bulk-paste-section fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '15px', width: '100%' }}>
                    <div className="form-group">
                      ...
                      <textarea className="modern-input" rows="12" value={bulkText} onChange={e => setBulkText(e.target.value)} style={{ fontFamily: 'monospace', fontSize: '13px' }} />
                    </div>
                    <div>
                      <button type="button" className="btn-primary" onClick={handleBulkParse}>Extract & Import Questions</button>
                    </div>
                  </div>
                )}
              </div>
                ...
                <div className="summary-box">
                    ...
                    {questions.map((q, i) => (
                      <div key={i} className="question-summary-item" style={{ position: 'relative', paddingRight: '40px' }}>
                        ...
                        <div style={{ position: 'absolute', top: '8px', right: '8px', display: 'flex', gap: '6px' }}>
                          <button type="button" onClick={() => handleEditQuestion(i)} style={{ background: 'none', border: 'none', color: '#3b82f6', cursor: 'pointer', padding: '2px' }}><Edit2 size={14} /></button>
                          <button type="button" onClick={() => handleDeleteQuestion(i)} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', padding: '2px' }}><Trash2 size={14} /></button>
                        </div>
                      </div>
                    ))}
                  </div>
                  {questions.length > 0 && (
                    <div style={{ display: 'flex', gap: '10px', marginTop: '16px' }}>
                      {editingTestId && (<button className="btn-secondary w-full" onClick={handleCancelEdit}>Cancel Edit</button>)}
                      <button className="btn-primary w-full" onClick={handleSaveTest}>{editingTestId ? 'Update Test' : 'Save Full Test'}</button>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '20px', borderBottom: '1px solid #e2e8f0', paddingBottom: '20px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <input type="email" className="modern-input" readOnly value="admin@nexora.com" style={{ backgroundColor: '#f8fafc' }} />
                  </div>
                </div>
              </div>
            </div>
